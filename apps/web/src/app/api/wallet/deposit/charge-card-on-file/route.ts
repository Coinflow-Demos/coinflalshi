...
import {getClientIp, chargeCoinflowCardOnFile} from '@/lib/coinflow/server';
...
export async function POST(request: Request) {
  ...
  const {amountCents, savedPaymentMethodId, authentication3DS, deviceId} = parsed.data;

  const savedPaymentMethod = await db.savedPaymentMethod.findUnique({where: {id: savedPaymentMethodId}});
  ...
  try {
    ...
    const result = await chargeCoinflowCardOnFile({
      userId,
    ...
    if (result.status === 'challenge') {
      // Stash what's needed to finish this charge after the 3DS challenge —
      // /complete reads it back from here instead of trusting the client.
      await db.transaction.update({
        where: {id: transaction.id},
        data: {metadata: {pendingCharge: {savedPaymentMethodId}}},
      });
