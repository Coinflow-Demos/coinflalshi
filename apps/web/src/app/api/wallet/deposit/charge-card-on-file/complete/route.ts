...
// What /charge-card-on-file stashed on the transaction when it returned a
// 3DS challenge — read back here instead of trusting whatever the client
// sends at completion time.
...
export async function POST(request: Request) {
  ...
  const savedPaymentMethod = await db.savedPaymentMethod.findUnique({
    where: {id: pendingCharge.data.savedPaymentMethodId},
  });
